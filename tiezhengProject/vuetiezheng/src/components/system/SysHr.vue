<template>
  <div style="margin-top: 10px" v-loading="fullloading">
    <div style="margin-bottom: 10px;display: flex;justify-content: center;align-items: center">
      ...
      <el-button size="small" type="primary" icon="el-icon-search" @click="addHr('zhanxiao','123456')">新增</el-button>
    </div>
...
<script>
  export default{
    ...
    methods: {
      ...
      addHr(username,password){
        var _this = this;
        this.fullloading = true;
        this.postRequest("/system/hr/hr/reg", this.account).then(resp=> {
          _this.fullloading = false;
          if (resp && resp.status == 200) {
            var data = resp.data;
            _
