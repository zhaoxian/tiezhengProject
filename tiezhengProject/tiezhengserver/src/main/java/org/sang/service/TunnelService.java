...
//    DecimalFormat decimalFormat = new DecimalFormat("##.00");
    ...
    public List<Tunnel> getTunnelByPage(Integer page, Integer size){
        int start = (page - 1) * size;
        return tunnelMapper.getTunnelsPage(start, size);
    }
    public int addTunnel(Tunnel tunnel) {
        return tunnelMapper.addTunnel(tunnel);
    }

    public int updateTunnel(Tunnel tunnel) {
        return tunnelMapper.updateTunnel(tunnel);
    }

    public boolean deleteTunnelById(String ids) {
        String[] split = ids.split(",");
        return tunnelMapper.deleteTunnelById(split) == split.length;
    }
