...
import org.sang.mapper.MeasurePointMapper;
import org.sang.mapper.SectionMapper;
import org.sang.mapper.TunnelMapper;
...
//    DecimalFormat decimalFormat = new DecimalFormat("##.00");
    ...
    public List<MeasurePoint> getMeasurePointByPage(Integer page, Integer size){
        int start = (page - 1) * size;
        return measurePointMapper.getMeasurePointsByPage(start, size);
    }
    public int addMeasurePoint(MeasurePoint measurePoint) {
        return measurePointMapper.addMeasurePoint(measurePoint);
    }

    public int updateMeasurePoint(MeasurePoint measurePoint) {
        return measurePointMapper.updateMeasurePoint(measurePoint);
    }

    public boolean deleteMeasurePointById(String ids) {
        String[] split = ids.split(",");
        return measurePointMapper.deleteMeasurePointById(split) == split.length;
    }
